...
import type { AitmplProviderConfig } from "../../config/schema.js";

describe("AITMPLProvider", () => {
  let provider: AITMPLProvider;
  let logger: Logger;

  const testConfig: AitmplProviderConfig = {
    enabled: true,
    apiUrl:
  ...
  describe("Lifecycle", () => {
    ...
    it("should handle disabled configuration", async () => {
      const disabledProvider = new AITMPLProvider(
  ...
  describe("fetchResource()", () => {
    ...
    it("should parse frontmatter from resource content", async () => {
      try {
        ...
        if (index.resources.length > 0) {
          const resource = index.resources[0];
          const fullResource = await provider.fetchResource(
            resource.id,
            resource.category,
          );
          ...
          assert.ok(
            fullResource.description !== undefined,
            "Should have description",
          );
          assert.ok(
            fullResource.estimatedTokens > 0,
  ...
  describe("healthCheck()", () => {
    ...
    it("should report unhealthy status on repeated failures", async () => {
      ...
      assert.ok(
        ...
        "Should report unhealthy/degraded status",
      );
    ...
    it("should cache resources independently", async () => {
      try {
        ...
        if (index.resources.length >= 2) {
          ...
          assert.ok(
            stats2.cachedRequests > stats1.cachedRequests,
            "Should cache resources independently",
          );
        }
      } catch (error) {
        if (error instanceof ProviderError) {
    ...
    it("should map AITMPL component types to categories", async () => {
      try {
        ...
        for (const resource of index.resources) {
          assert.ok(
            ["agent", "skill", "example", "pattern", "workflow"].includes(
              resource.category,
            ),
            `Category ${resource.category} should be valid`,
          );
        }
      } catch (error) {
        if (error instanceof ProviderError) {
    ...
    it("should estimate tokens reasonably", async () => {
      try {
        ...
        if (index.resources.length > 0) {
          for (const resource of index.resources) {
            assert.ok(
              resource.estimatedTokens > 0,
              "Token estimate should be positive",
            );
            assert.ok(
              resource.estimatedTokens < 1000000,
