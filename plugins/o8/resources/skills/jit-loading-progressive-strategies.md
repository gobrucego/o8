...
**→ Load:** `o8://skills/match?query=performance+optimization+${tech}&mode=index&maxResults=5`
...
**→ Load:** `o8://skills/match?query=security+${threat-model}+${tech}&mode=index&maxResults=5`

## Phase 6: Deployment [CONDITIONAL]
Only if user requests deployment:
**→ Load:** `o8://skills/match?query=deployment+${platform}+${tech}&mode=index&maxResults=5`
```
...
**→ Load:** `o8://patterns/match?query=architecture+${type}&mode=index&maxResults=5`
General architectural patterns

## Phase 2: Detailed design (20-40%)
**→ Load:** `o8://agents/match?query=${specific-tech}+${component}&mode=index&maxResults=8`
...
**→ Load:** `o8://patterns/match?query=api+design+rest&mode=index&maxResults=3`
General REST API patterns

## Phase 2: Framework Implementation (20-60%)
**→ Load:** `o8://agents/match?query=${framework}+api+implementation&mode=index&maxResults=8`
...
**→ Load once:** `o8://match?query=${tech-stack}+${architecture}&categories=agent,skill,pattern&mode=index&maxResults=12`
...
**→ Comprehensive Load:**
o8://match?query=${tech}+${domain}+full-stack&categories=agent,skill,example&mode=index&maxResults=15
...
**→ Load:** `o8://match?query=sql+${database}+orm&mode=index&maxResults=8`

**Else if user selected NoSQL:**
**→ Load:** `o8://match?query=nosql+${database}+modeling&mode=index&maxResults=7`

**Else if user selected in-memory:**
**→ Load:** `o8://match?query=cache+${cache-tech}+patterns&mode=index&maxResults=5`
...
**→ Load:** `o8://match?query=${tech}+crud+basic&mode=index&maxResults=8`

**If complex business logic:**
Increase budget: 3000 tokens
**→ Load:** `o8://match?query=${tech}+${domain}+complex&mode=index&maxResults=12`

**If microservices:**
Split across sub-phases: 2 × 1500 tokens
**→ Load Service A:** `o8://match?query=${tech}+${service-a}&mode=index&maxResults=8`
**→ Load Service B:** `o8://match?query=${tech}+${service-b}&mode=index&maxResults=8`
...
o8://match?query=${specific-requirements}&mode=index&maxResults=8
