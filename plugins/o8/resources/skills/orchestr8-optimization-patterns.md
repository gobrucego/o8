...
**→ Load:** o8://match?query=requirement+analysis&mode=index&maxResults=5
...
**→ Load:** o8://match?query=typescript+api+implementation&mode=index&maxResults=8
...
**→ Load:** o8://match?query=testing+validation&mode=index&maxResults=3
...
└─ Reference: o8://agents/typescript-expert-advanced
...
→ o8://agents/typescript-expert
Tokens: 600

# Advanced case: Load both
→ o8://agents/typescript-expert
→ o8://agents/typescript-expert-advanced
...
const query = "o8://match?query=async+patterns&mode=index&maxResults=5"
...
const specific = "o8://skills/typescript-async-patterns"
...
**→ Load:** o8://match?query=system+analysis&mode=index&maxResults=5

**If needed (conditional):**
**→ Load:** o8://patterns/architecture-analysis (add 600 tokens)
...
relatedResources:
  ...
  - o8://skills/testing-async-patterns
  
  # Related patterns (design approaches)
  - o8://patterns/async-concurrency-patterns
  
  # Related examples (practical code)
  - o8://examples/typescript-async-api-client
...
**→ Load:** o8://match?query=project+planning&mode=index&maxResults=3
...
**→ Load:** o8://match?query=typescript+backend+api&mode=index&maxResults=8
...
**→ Load:** o8://match?query=e2e+testing+playwright&mode=index&maxResults=5
...
**→ Load:** o8://agents/full-expert
**→ Load:** o8://skills/all-skills
**→ Load:** o8://patterns/all-patterns
...
**→ Load:** o8://match?query=typescript
...
**→ Load:** o8://match?query=typescript+api+architecture+setup&mode=index&maxResults=5
...
**→ Load:** o8://match?query=express+routes+validation+error+handling&mode=index&maxResults=8
...
**→ Load:** o8://match?query=api+testing+jest+supertest&mode=index&maxResults=5
...
**→ Load:** o8://match?query=api+documentation+openapi&mode=index&maxResults=3
