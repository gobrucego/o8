...
useWhen:
  ...
  - Building o8:// URI scheme for agent and skill discovery enabling declarative workflow composition
...
Construct dynamic o8:// URIs for query-based resource loading using fuzzy matching and filtering.
...
```markdown
o8://agents/match?query=${project-type}+${technology}
o8://skills/match?query=${technique}+${context}
...
Phase 1:
o8://agents/match?query=research+api+requirements&mode=index&maxResults=3

Phase 2:
o8://agents/match?query=typescript+api+rest+backend&mode=index&maxResults=8

Phase 3:
o8://skills/match?query=jwt+authentication+typescript&mode=index&maxResults=5
```
...
o8://match?query=python+fastapi+postgresql+jwt&categories=agent,skill&mode=index&maxResults=10
...
o8://agents/match?query=${domain}
→ Load expert agents only

Middle phases (implementation):
o8://match?query=${tech}+${feature}&categories=agent,skill,example
→ Load agents + skills + code examples

Late phases (testing/deployment):
o8://skills/match?query=testing+deployment+${tech}
→ Load skills only
```
...
o8://skills/match?query=security+authentication+authorization&tags=jwt,oauth&mode=index&maxResults=8
...
## Phase 1
**→ Reference:** `o8://match?query=${project-description}&categories=agent&mode=index&maxResults=8`

## Phase 2
**→ Reference:** `o8://match?query=${tech-stack}+architecture&categories=pattern&mode=index&maxResults=5`
...
If building API:
**→ Load:** `o8://match?query=${language}+api+rest&mode=index&maxResults=8`

If building CLI:
**→ Load:** `o8://match?query=${language}+cli+command&mode=index&maxResults=8`

If building web app:
**→ Load:** `o8://match?query=${language}+web+frontend&mode=index&maxResults=10`
...
o8://match?query=${tech-stack}+api+architecture&categories=agent,pattern&mode=index&maxResults=8
...
o8://match?query=${tech}+${features}+${auth-method}&categories=agent,skill,example&mode=index&maxResults=10
...
o8://skills/match?query=testing+deployment+api+${tech}&mode=index&maxResults=5
