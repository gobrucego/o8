...
category: skill
tags: [tdd, testing, red-green-refactor, unit-tests, test-first, quality]
capabilities:
  ...
  - Test behavior not implementation patterns
  - TDD workflows for new features, bug fixes, and refactoring
  - Multi-language TDD with Jest, pytest, JUnit, Go testing
estimatedTokens: 620
useWhen:
  - Implementing new features requiring test-first approach with Red-Green-Refactor cycle where failing test is written first, then minimal code to pass, then refactoring
  - Fixing bugs requiring regression test creation by writing test that reproduces bug (RED), fixing the bug (GREEN), verifying test passes, and adding edge case tests
  - Refactoring code while maintaining test coverage by ensuring tests exist and pass (GREEN), refactoring code, running tests frequently, keeping tests GREEN throughout
  - Building business logic and complex algorithms requiring comprehensive test coverage, one assertion per test (mostly), and test behavior not implementation
  - Creating API endpoints with contract testing using AAA pattern (Arrange, Act, Assert) and proper mocking with test doubles (mocks, stubs, spies)
  - Writing security-critical code requiring validation through test-driven approach with edge case testing, async code testing, and flaky test prevention
---
...
### ❌ Anti-Pattern 2: Write Tests After Code
