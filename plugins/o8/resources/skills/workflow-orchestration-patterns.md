...
category: skill
tags: [workflows, orchestration, multi-phase, quality-gates, agent-coordination, checkpoints]
capabilities:
  - Multi-phase execution design with percentage allocation
  - Quality gate implementation (parallel, sequential, conditional)
  - Agent coordination patterns and delegation strategies
  - Success criteria definition and checkpoint validation
  - Workflow frontmatter structure and argumentHint design
estimatedTokens: 780
useWhen:
  - Designing multi-phase workflow slash commands requiring phase percentages that add to 100%, specific agent assignments, and checkpoint validation at boundaries
  - Implementing quality gates with parallel execution for independent checks (code review, testing, security, performance) requiring all gates must PASS validation
  - Coordinating multiple agents in workflows using sequential dependencies, parallel independent tracks, conditional agent selection, or orchestrator delegation patterns
  - Defining 8-12 specific measurable success criteria covering acceptance, testing, quality, security, documentation, deployment, monitoring, and user validation
  - Creating workflow frontmatter with description following action verb + scope + capabilities + benefits pattern and optional argumentHint for parameters
  - Building autonomous workflows with clear phase objectives, explicit agent names (not generic), parallelization declarations, and no escape hatches for quality gates
---
...
### Pattern 2: Conditional Quality Gates
