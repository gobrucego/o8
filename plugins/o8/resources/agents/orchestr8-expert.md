...
relatedResources:
  ...
  - o8://skills/fragment-metadata-optimization
  - o8://skills/jit-loading-progressive-strategies
  - o8://skills/jit-loading-phase-budgets
  - o8://patterns/dynamic-expertise-core
---
...
On-demand: o8://match queries load 2-5 fragments (~1,500 tokens)
...
**→ Load:** o8://match?query=typescript+setup&mode=index&maxResults=5

## Phase 2: Implementation (30-70%)
**→ Load:** o8://match?query=async+error+handling&mode=index&maxResults=8
```
...
└─ Reference: o8://agents/agent-name-advanced
...
**User Flow:**
```
1. Query: o8://match?query=typescript+async&mode=index&maxResults=5
   Returns: 5 fragments, ~80 tokens, URIs only

2. User reviews resource names and quick descriptions

3. Load specific: o8://skills/typescript-async-patterns
...
- o8://skills/related-skill
- o8://patterns/related-pattern
```
...
**→ Load:** o8://match?query=specific+keywords&mode=index&maxResults=5
...
**→ Load:** o8://match?query=different+keywords&mode=index&maxResults=8
...
**→ Load:** o8://match?query=testing+keywords&mode=index&maxResults=3
...
o8://match?query=discovery+keywords&mode=index&maxResults=3

# Phase 1: Core Work
o8://match?query=core+keywords&mode=index&maxResults=5

# Phase 2: Advanced (if needed)
o8://agents/specific-expert-advanced
​```
...
**→ Load:** o8://match?query=phase1+keywords&mode=index&maxResults=5
...
**→ Load (if needed):** o8://agents/advanced-expert
...
1. **Use JIT Loading in Workflows**
   ```markdown
   **→ Load:** o8://match?query=keywords&mode=index&maxResults=5
   ```
...
4. **Leverage Cross-References**
   - Link related fragments with `o8://` URIs
...
1. **Don't Load Everything Upfront**
   ```markdown
   # ❌ BAD
   **→ Load:** o8://agents/full-agent
   **→ Load:** o8://skills/all-skills
...
□ JIT loading: Commands/workflows use o8://match
...
relatedResources:
  ...
  - o8://patterns/async-patterns
  - o8://examples/typescript-async-examples
---
...
- o8://skills/error-handling-async - Async error strategies
- o8://patterns/async-patterns - Design patterns
```
