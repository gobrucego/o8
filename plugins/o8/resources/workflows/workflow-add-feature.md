...
```
o8://match?query=codebase+analysis+feature+design+${tech-stack}&categories=skill,pattern&mode=index&maxResults=5
...
```
o8://match?query=${tech-stack}+${feature-type}+implementation&categories=agent,skill,example&mode=index&maxResults=10
```
...
```
o8://match?query=${tech-stack}+testing+security+quality&categories=skill,agent&mode=index&maxResults=8
o8://skills/testing-e2e-best-practices
o8://skills/security-input-validation
o8://skills/quality-code-review-checklist
```
...
o8://match?query=${platform}+deployment+feature+flags+monitoring&categories=skill,pattern&mode=index&maxResults=5
