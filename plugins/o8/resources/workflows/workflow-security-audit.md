...
```
o8://match?query=security+threat+modeling+attack+surface+${tech-stack}&categories=skill,pattern&mode=index&maxResults=5
...
```
o8://match?query=owasp+security+vulnerabilities+${tech-stack}&categories=skill,agent&mode=index&maxResults=10
o8://skills/security-owasp-top10
o8://agents/security-owasp-vulnerabilities
```
...
o8://match?query=${package-manager}+dependency+audit+cve&categories=skill&mode=index&maxResults=3
```
...
o8://match?query=${cloud-provider}+infrastructure+security+hardening&categories=skill,pattern&mode=index&maxResults=5
...
o8://match?query=${compliance-standard}+data+protection+encryption&categories=skill,agent&mode=index&maxResults=5
```
...
```
o8://match?query=security+remediation+reporting+cvss&categories=skill&mode=index&maxResults=3
