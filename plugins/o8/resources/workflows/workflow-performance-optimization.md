...
**Token Efficiency:**
- Without JIT: ~15,000 tokens upfront (all performance optimization resources)
- With JIT: ~3,200 tokens progressive (loaded by optimization phase)
- **Savings: 79%**
...
```
o8://match?query=performance+profiling+bottleneck+analysis+${tech-stack}&categories=skill,agent&mode=index&maxResults=8
o8://skills/performance-profiling-techniques
```
...
o8://match?query=${bottleneck-type}+optimization+${tech-stack}&categories=skill,pattern&mode=index&maxResults=12
```
...
```
o8://match?query=${database-type}+query+optimization+indexing&categories=skill,agent&mode=index&maxResults=5
o8://skills/performance-database-optimization
```
...
```
o8://match?query=${backend-tech}+performance+caching+async&categories=skill,agent&mode=index&maxResults=5
o8://skills/performance-api-optimization
```
...
```
o8://match?query=${frontend-framework}+performance+rendering+bundle&categories=skill,agent&mode=index&maxResults=5
o8://skills/performance-frontend-optimization
```
...
```
o8://match?query=${platform}+infrastructure+scaling+cdn&categories=skill,pattern&mode=index&maxResults=5
o8://skills/performance-caching
```
...
```
o8://match?query=performance+testing+monitoring+benchmarking+${tech-stack}&categories=skill,pattern&mode=index&maxResults=8
...
**Conditional Loading by Bottleneck Type:**
1. Phase 1: Always load profiling expertise (1500 tokens)
2. Phase 2: Load ONLY the optimization expertise needed for identified bottlenecks:
   - Database bottleneck? Load database optimization (1200 tokens)
   - Backend bottleneck? Load backend optimization (1200 tokens)
   - Frontend bottleneck? Load frontend optimization (1200 tokens)
   - Infrastructure bottleneck? Load infrastructure optimization (1000 tokens)
3. Phase 3: Load testing and monitoring expertise (1500 tokens)

**Adaptive Budget:**
- Minimal (1 bottleneck): ~4,200 tokens (profiling + 1 optimization + validation)
- Typical (2-3 bottlenecks): ~6,000-7,500 tokens
- Comprehensive (all layers): ~9,600 tokens

**Token Savings:**
- Traditional approach: Load all optimization expertise upfront = ~15,000 tokens
- JIT approach: Load only needed layers = ~4,200-9,600 tokens
- **Savings: 36-72% depending on bottleneck scope**
