...
useWhen:
  - Variable requirements across workflow invocations requiring JIT resource loading with o8:// URIs and fuzzy matching queries
...
```markdown
Always load:
- o8://agents/typescript-developer (1500 tokens)
- o8://skills/testing (1000 tokens)
- o8://patterns/rest-api (800 tokens)
...
o8://match?query=typescript+async+error+handling&mode=index&maxResults=8
...
o8://examples/typescript/rest-api
...
```markdown
Single-category:
o8://agents/match?query=python+fastapi+api&mode=index&maxResults=8
o8://skills/match?query=error+handling+async&mode=index&maxResults=5

Multi-category:
o8://match?query=typescript+api+auth&categories=agent,skill,example&mode=index&maxResults=10

Tag-filtered:
o8://skills/match?query=testing&tags=integration,api&mode=index&maxResults=8
...
Example:
o8://skills/match?query=error+handling+async+validation&mode=index&maxResults=5
...
Example:
o8://patterns/match?query=microservices+event+driven&mode=index&maxResults=5
...
Example:
o8://match?query=typescript+api+rest+auth+testing&categories=agent,skill,pattern,example&mode=index&maxResults=15
...
Example:
o8://match?query=python+data+pipeline&categories=agent,skill&mode=index&maxResults=8
...
Construct query:
o8://match?query=typescript+real-time+websockets+chat&categories=agent,skill,example&mode=index&maxResults=10
```
...
o8://agents/match?query=research+${domain}&mode=index&maxResults=3

Phase 1 determines: Need Python + FastAPI + PostgreSQL

## Phase 2: Targeted Implementation
o8://match?query=python+fastapi+postgresql+api&categories=agent,skill&mode=index&maxResults=10
...
Query 1 (core):
o8://match?query=api+authentication+${tech}&mode=index&maxResults=8

Query 2 (enhancements):
o8://skills/match?query=caching+rate-limiting+logging&mode=index&maxResults=5
...
Query:
o8://match?query=database+performance+optimization+query+indexing&categories=agent,skill,pattern&mode=index&maxResults=8
...
### Adaptive Budgeting
```markdown
Simple task: mode=index&maxResults=8
o8://match?query=${simple-crud-api}&mode=index&maxResults=8

Medium complexity: mode=index&maxResults=10
o8://match?query=${multi-service-integration}&mode=index&maxResults=10

High complexity: mode=index&maxResults=15
o8://match?query=${distributed-system-event-driven}&mode=index&maxResults=15
```
...
o8://match?query=${tech}+architecture&categories=agent,pattern&mode=index&maxResults=8

## Phase 3: Implementation (largest budget)
o8://match?query=${tech}+${features}&categories=agent,skill,example&mode=index&maxResults=10

## Phase 4: Testing (focused budget)
o8://skills/match?query=testing+${tech}&mode=index&maxResults=5
```
