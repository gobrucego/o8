...
useWhen:
  - Adaptive workflow creation requiring dynamic o8:// URIs with query parameters determined by user input or prior phase results
...
**→ Research Assembly:**
o8://agents/match?query=research+requirements&mode=index&maxResults=3
...
o8://match?query=${tech-stack}+${architecture}&categories=agent,pattern&mode=index&maxResults=8
...
o8://match?query=${tech-stack}+${features}+${patterns}&categories=agent,skill,example&mode=index&maxResults=10
...
**→ Testing Assembly:**
o8://skills/match?query=testing+${tech-stack}&mode=index&maxResults=5
...
o8://skills/api-development
o8://patterns/rest-architecture
...
o8://match?query=${task-description}&categories=agent,skill,pattern&mode=index&maxResults=10
...
o8://match?query=python+fastapi+postgresql+jwt&categories=agent,skill,example&mode=index&maxResults=10
...
## Phase 1: Core (0-40%)
**→ Core Expertise:**
o8://agents/match?query=${tech}+core&mode=index&maxResults=5

## Phase 2: Features (40-75%)
**→ Feature-Specific:**
o8://match?query=${tech}+${features}&categories=skill,example&mode=index&maxResults=8

## Phase 3: Polish (75-100%)
**→ Quality & Testing:**
o8://skills/match?query=testing+optimization+${tech}&mode=index&maxResults=5
```
...
o8://match?query=${full-stack-description}&categories=agent,skill,pattern&mode=index&maxResults=15
...
Direct use in URI:
o8://match?query=${task-description}&mode=index&maxResults=8

Expands to:
o8://match?query=Build+TypeScript+REST+API&mode=index&maxResults=8
...
o8://match?query=${tech}+${pattern}+${features}&mode=index&maxResults=10
...
**→ Query 1:**
o8://agents/match?query=${domain}+expert&mode=index&maxResults=8

## Phase 2: Specific Skills
**→ Query 2:**
o8://skills/match?query=${specific-technique}+${tech}&mode=index&maxResults=5
...
o8://agents/match?query=${tech}+${domain}&mode=index&maxResults=8

**→ Supplementary Query (techniques):**
o8://skills/match?query=${pattern}+${technique}&mode=index&maxResults=5
...
**→ Query:**
o8://match?query=python+microservice+payment+stripe&categories=agent,pattern&mode=index&maxResults=8
...
o8://match?query=python+fastapi+stripe+api+integration&categories=agent,skill,example&mode=index&maxResults=10
...
**→ Query:**
o8://skills/match?query=testing+python+fastapi+integration&mode=index&maxResults=5
