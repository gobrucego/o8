...
export class ConfigLoader {
  ...
  async load(): Promise<Config> {
    ...
    if (!validationResult.success) {
      ...
      throw new Error(
        `Invalid configuration: ${validationResult.errors?.map((e) => `${e.path}: ${e.message}`).join(", ")}`
      );
    }
   ...
   */
  private loadUserConfig(): Partial<Config> | null {
   ...
   */
  private loadEnvConfig(): Partial<Config> | null {
    const envConfig: any = {
      resourceProviders: {},
      providerDefaults: {},
    };
    ...
    if (process.env.ORCHESTR8_AITMPL_CACHE_TTL) {
      envConfig.resourceProviders.aitmpl = envConfig.resourceProviders.aitmpl || {};
      envConfig.resourceProviders.aitmpl.cacheTTL = parseInt(
        process.env.ORCHESTR8_AITMPL_CACHE_TTL,
        10
      );
      hasEnvConfig = true;
    }
    ...
    if (process.env.ORCHESTR8_AITMPL_TIMEOUT) {
      envConfig.resourceProviders.aitmpl = envConfig.resourceProviders.aitmpl || {};
      envConfig.resourceProviders.aitmpl.timeout = parseInt(
        process.env.ORCHESTR8_AITMPL_TIMEOUT,
        10
      );
      hasEnvConfig = true;
    }
    ...
    if (process.env.ORCHESTR8_AITMPL_RETRY_ATTEMPTS) {
      envConfig.resourceProviders.aitmpl = envConfig.resourceProviders.aitmpl || {};
      envConfig.resourceProviders.aitmpl.retryAttempts = parseInt(
        process.env.ORCHESTR8_AITMPL_RETRY_ATTEMPTS,
        10
      );
      hasEnvConfig = true;
    }
    ...
    if (process.env.ORCHESTR8_GITHUB_TOKEN) {
      ...
      envConfig.resourceProviders.github.auth = {
        ...
        type: (process.env.ORCHESTR8_GITHUB_TOKEN_TYPE as "personal" | "oauth") || "personal",
      };
      hasEnvConfig = true;
    }
    ...
    if (process.env.ORCHESTR8_GITHUB_CACHE_TTL) {
      envConfig.resourceProviders.github = envConfig.resourceProviders.github || {};
      envConfig.resourceProviders.github.cacheTTL = parseInt(
        process.env.ORCHESTR8_GITHUB_CACHE_TTL,
        10
      );
      hasEnvConfig = true;
    }
    ...
    if (process.env.ORCHESTR8_PROVIDER_PRIORITY) {
      envConfig.providerDefaults.priority = parseInt(
        process.env.ORCHESTR8_PROVIDER_PRIORITY,
        10
      );
      hasEnvConfig = true;
    }
    ...
    if (process.env.ORCHESTR8_PROVIDER_CACHE_TTL) {
      envConfig.providerDefaults.cacheTTL = parseInt(
        process.env.ORCHESTR8_PROVIDER_CACHE_TTL,
        10
      );
      hasEnvConfig = true;
    }
    ...
    if (process.env.ORCHESTR8_PROVIDER_TIMEOUT) {
      envConfig.providerDefaults.timeout = parseInt(
        process.env.ORCHESTR8_PROVIDER_TIMEOUT,
        10
      );
      hasEnvConfig = true;
    }
    ...
    if (process.env.ORCHESTR8_PROVIDER_RETRY_ATTEMPTS) {
      envConfig.providerDefaults.retryAttempts = parseInt(
        process.env.ORCHESTR8_PROVIDER_RETRY_ATTEMPTS,
        10
      );
      hasEnvConfig = true;
    }
   ...
   */
  private mergeConfigs(): Partial<Config> {
  ...
  private logConfigSummary(): void {
    ...
    if (resourceProviders.aitmpl.enabled) enabledProviders.push("aitmpl");
    if (resourceProviders.github.enabled) enabledProviders.push("github");
    if (resourceProviders.custom.enabled) enabledProviders.push("custom");
  ...
  async updateConfig(updates: Partial<Config>): Promise<Config> {
    ...
    if (!validationResult.success) {
      ...
      throw new Error(
        `Invalid configuration update: ${validationResult.errors?.map((e) => `${e.path}: ${e.message}`).join(", ")}`
      );
    }
   ...
   */
  getProviderConfig<T extends "aitmpl" | "github" | "custom">(
    provider: T
  ): Config["resourceProviders"][T] {
    if (!this.config) {
      throw new Error("Configuration not loaded. Call load() first.");
    }
