/**
 * ResourceLoader Provider Integration Tests
 *
 * Tests the integration between ResourceLoader and the provider system:
 * - Provider initialization
 * - Multi-provider search
 * - Resource fetching from different providers
...
describe("ResourceLoader Provider Integration", () => {
  ...
  describe("Provider Initialization", () => {
    ...
    it("should handle provider initialization failures gracefully", async () => {
      // Initialization should not throw even if remote providers fail
      await resourceLoader.initializeProviders();

      const providers = resourceLoader.getProviderNames();
      // Should at least have local provider
    ...
    it("should respect provider configuration", async () => {
      ...
      // All enabled providers should be listed
  ...
  describe("Multi-Provider Search", () => {
    ...
    it("should search across all providers", async () => {
      ...
      // Should have at least some results from local provider
    });
    ...
    it("should handle queries with no matches", async () => {
      const results = await resourceLoader.searchAllProviders(
        "xyzzzyqwertnonexistentqueryabc123456789",
      );

      assert.ok(
        Array.isArray(results),
        "Should return array even with no matches",
      );
      // May have no results, which is fine
    });
  ...
  describe("Single Provider Search", () => {
    ...
    it("should search in local provider", async () => {
      const results = await resourceLoader.searchProvider(
        "local",
        "typescript",
        {
          maxResults: 10,
  ...
  describe("Resource Fetching", () => {
    ...
    it("should load resource from local provider via o8:// URI", async () => {
      try {
        ...
        // May fail if no resources available
        console.log("Resource fetch test skipped:", (error as Error).message);
      }
    });
    ...
    it("should cache loaded resources", async () => {
      try {
        ...
        console.log("Cache test skipped:", (error as Error).message);
      }
    });
    ...
    it("should handle dynamic resource URIs", async () => {
      try {
        ...
        // May fail depending on resource availability
        console.log("Dynamic URI test skipped:", (error as Error).message);
      }
    });
  });
  ...
  describe("Health Monitoring", () => {
    ...
    it("should include health metrics", async () => {
      ...
      if (health.local) {
        assert.ok(
          ["healthy", "degraded", "unhealthy"].includes(health.local.status),
          "Should have valid status",
        );
        assert.ok(
          health.local.lastCheck instanceof Date,
          "Should have check timestamp",
        );
      }
    });
    ...
    it("should detect unhealthy providers", async () => {
      ...
      // At least local should be healthy
      assert.strictEqual(
        health.local?.status,
        "healthy",
        "Local provider should be healthy",
      );
    });
  });
  ...
  describe("Statistics", () => {
    ...
    it("should get aggregate statistics", () => {
      ...
      assert.ok(
        typeof aggStats.totalProviders === "number",
        "Should have total providers count",
      );
      assert.ok(
        typeof aggStats.enabledProviders === "number",
        "Should have enabled providers count",
      );
    ...
    it("should aggregate metrics across providers", async () => {
      ...
      assert.ok(
        aggStats.aggregate.totalRequests >= 0,
        "Should aggregate requests",
      );
    });
  });
  ...
  describe("Provider Management", () => {
    ...
    it("should get index from specific provider", async () => {
      try {
        ...
        console.log("Index fetch skipped:", (error as Error).message);
      }
    });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle provider not found errors", async () => {
      await assert.rejects(
        ...
        Error,
        "Should throw for non-existent provider",
      );
    });
    ...
    it("should handle resource not found errors", async () => {
      await assert.rejects(
        ...
        Error,
        "Should throw for non-existent resource",
      );
    });
  ...
  describe("Backward Compatibility", () => {
    ...
    it("should load resources without provider initialization", async () => {
      ...
      // Should work with local filesystem even without providers
      try {
        const resources = await newLoader.loadAllResources();
        assert.ok(Array.isArray(resources), "Should load resources");
      } catch (error) {
        console.log("Compatibility test skipped:", (error as Error).message);
      }
    });
    ...
    it("should support legacy resource loading methods", async () => {
      try {
        ...
        console.log("Legacy test skipped:", (error as Error).message);
      }
    });
    ...
    it("should support legacy search methods", async () => {
      try {
        ...
        console.log("Legacy search skipped:", (error as Error).message);
      }
    });
  });
  ...
  describe("Performance", () => {
    ...
    it("should initialize providers in reasonable time", async () => {
      ...
      assert.ok(
        duration < 10000,
        `Provider initialization should be fast (<10s), took ${duration}ms`,
      );
    });
    ...
    it("should perform multi-provider search quickly", async () => {
      ...
      assert.ok(
        duration < 5000,
        `Multi-provider search should be fast (<5s), took ${duration}ms`,
      );
    });
    ...
    it("should benefit from caching", async () => {
      ...
      // But we can't guarantee it, so just verify both complete
  ...
  describe("Integration Scenarios", () => {
    ...
    it("should handle complete workflow: search -> fetch -> use", async () => {
      try {
        ...
        console.log("Workflow test skipped:", (error as Error).message);
      }
    });
    ...
    it("should handle provider failure gracefully in workflow", async () => {
      ...
      try {
        ...
        // Search should still work with remaining providers
        const results = await resourceLoader.searchAllProviders("test");
