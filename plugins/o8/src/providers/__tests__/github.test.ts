 ...
 *
 * Tests the GitHubProvider implementation including:
 * - Initialization with multiple repositories
 * - Repository scanning and structure detection
 * - Index building with tree API
 * - Resource fetching from raw content
...
describe("GitHubProvider", () => {
  ...
  const testConfig: GithubProviderConfig = {
    enabled: true,
    repos: ["davila7/claude-code-templates"], // Known public repo
    branch: "main",
  ...
  describe("Lifecycle", () => {
    ...
    it("should validate repository format during initialization", async () => {
      const badProvider = new GitHubProvider(
        {
          ...testConfig,
          repos: ["invalid-format"], // Missing owner/repo structure
        },
        logger,
      );
      ...
      await assert.rejects(
        ...
        ProviderError,
        "Should reject invalid repository format",
      );
    });
    ...
    it("should check rate limit during initialization", async () => {
      try {
        ...
        // Rate limit should be populated if network is available
        if (stats.rateLimit) {
          assert.ok(
            typeof stats.rateLimit.remaining === "number",
    ...
    it("should handle authentication token if provided", async () => {
      // Test without actually providing a token (would need valid token)
      ...
      // Initialization will fail with invalid token, but tests error handling
      try {
  ...
  describe("fetchIndex()", () => {
    ...
    it("should scan multiple repositories if configured", async () => {
      const multiRepoProvider = new GitHubProvider(
        {
          ...
          repos: [
            ...
            // Add another repo here if needed for testing
          ],
        },
        logger,
      );
      ...
      try {
        ...
        // Should aggregate resources from all repos
    ...
    it("should include valid resource metadata", async () => {
      try {
        ...
        if (index.resources.length > 0) {
          ...
          assert.ok(
            resource.sourceUri.includes("github.com"),
            "Should have GitHub URI",
          );
        }
      } catch (error) {
        if (error instanceof ProviderError) {
    ...
    it("should detect repository structure", async () => {
      try {
        ...
        // Repository should be scanned and structure detected
    ...
    it("should cache index results", async () => {
      try {
        ...
        assert.ok(
          stats2.cachedRequests > stats1.cachedRequests,
    ...
    it("should include statistics in index", async () => {
      try {
        ...
        assert.ok(
          typeof index.stats.totalTokens === "number",
    ...
    it("should handle repository not found", async () => {
      ...
      try {
        ...
        // Should return empty index or partial results
        ...
        // May throw ProviderError
        assert.ok(
          error instanceof ProviderError,
          "Should handle repo not found",
        );
      }
  ...
  describe("fetchResource()", () => {
      ...
      await assert.rejects(
        async () => {
          await provider.fetchResource(
            "owner/repo/nonexistent-xyz-123.md",
            "agent",
          );
        },
        (error: any) => {
          assert.ok(
            error instanceof ResourceNotFoundError || error instanceof ProviderError,
          );
          return true;
        },
    ...
    it("should handle invalid resource ID format", async () => {
      await assert.rejects(
        ...
        ProviderError,
        "Should reject invalid resource ID format",
      );
    });
  });
  ...
  describe("search()", () => {
    ...
    it("should respect maxResults option", async () => {
      try {
        ...
        assert.ok(
          response.results.length <= maxResults,
          `Should return at most ${maxResults} results`,
        );
      } catch (error) {
        if (error instanceof ProviderError) {
          console.log("Network test skipped:", error.message);
      ...
      try {
        ...
        if (response.results.length > 0) {
          ...
          if (result.matchReason) {
            assert.ok(
              Array.isArray(result.matchReason),
              "Match reason should be an array",
            );
          }
        }
      } catch (error) {
    ...
    it("should handle queries with no matches", async () => {
      try {
        const response = await provider.search(
          "xyzzzyqwertnonexistentqueryabc123",
        );
      ...
      try {
        ...
        if (response.results.length > 1) {
          for (let i = 0; i < response.results.length - 1; i++) {
            assert.ok(
              response.results[i].score >= response.results[i + 1].score,
  ...
  describe("healthCheck()", () => {
    ...
    it("should check rate limit status", async () => {
      try {
        ...
        // Health should consider rate limit
        const stats = provider.getStats();
        if (stats.rateLimit) {
          assert.ok(
            typeof stats.rateLimit.remaining === "number",
    ...
    it("should include metrics after operations", async () => {
      try {
        ...
        if (health.metrics) {
          assert.ok(
            typeof health.metrics.successRate === "number",
  ...
  describe("getStats()", () => {
    ...
    it("should track API requests", async () => {
      ...
      try {
        ...
        assert.ok(
          statsAfter.totalRequests >= statsBefore.totalRequests,
    ...
    it("should track cache hits", async () => {
      try {
        ...
        assert.ok(
          statsAfter.cachedRequests > statsBefore.cachedRequests,
          "Cached requests should increase",
        );
      } catch (error) {
        if (error instanceof ProviderError) {
          console.log("Network test skipped:", error.message);
    ...
    it("should include rate limit information if available", () => {
      ...
      if (stats.rateLimit) {
        ...
        assert.ok(
          typeof stats.rateLimit.limit === "number",
          "Should have limit",
        );
        assert.ok(
          stats.rateLimit.resetAt instanceof Date,
  ...
  describe("Error Handling", () => {
    ...
    it("should handle network timeouts", async () => {
      ...
      await assert.rejects(
        ...
        ProviderError,
        "Should throw ProviderError on timeout",
      );
    ...
    it("should retry on transient failures", async () => {
      ...
      try {
        ...
        assert.ok(
          error instanceof ProviderError,
          "Should throw ProviderError",
        );
      }
    });

    it("should handle rate limit errors", async () => {
      // This test would require hitting actual rate limit
    ...
    it("should track failures in stats", async () => {
      ...
      // Stats should track the attempt
      assert.ok(
        statsAfter.totalRequests >= statsBefore.totalRequests,
  ...
  describe("Repository Structure Detection", () => {
    it("should detect orchestr8 structure", async () => {
      // This would require a repo with orchestr8 structure
    ...
    it("should detect claude-code-templates structure", async () => {
      try {
        ...
        // Should successfully parse the repo structure
    ...
    it("should handle flat structure repositories", async () => {
      // Flat structure should work as fallback
  ...
  describe("Caching", () => {
    ...
    it("should cache tree responses", async () => {
      try {
        // First fetch (will fetch tree)
        await provider.fetchIndex();

        // Second fetch (should use cached tree)
        await provider.fetchIndex();

        const stats = provider.getStats();
        assert.ok(
          stats.cachedRequests > 0,
          "Should cache tree responses",
        );
      } catch (error) {
        if (error instanceof ProviderError) {
          console.log("Network test skipped:", error.message);
  ...
  describe("Configuration", () => {
    it("should use custom branch if specified", async () => {
      ...
      // Should not throw error even if branch doesn't exist
      ...
      const multiRepoProvider = new GitHubProvider(
        {
          ...
          repos: [
            "davila7/claude-code-templates",
            "davila7/claude-code-templates", // Same repo twice to test deduplication
          ],
        },
        logger,
      );
