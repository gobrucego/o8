 ...
 *
 * Tests the ProviderRegistry implementation including:
...
class MockProvider implements ResourceProvider {
  ...
  setFailNextOperation(fail: boolean) {
...
describe("ProviderRegistry", () => {
  ...
  beforeEach(() => {
    ...
    registry = new ProviderRegistry({
      enableHealthChecks: false, // Disable auto health checks in tests
  ...
  describe("Provider Registration", () => {
    ...
    it("should reject duplicate provider names", async () => {
      ...
      await assert.rejects(
        ...
        /already registered/,
        "Should reject duplicate provider name",
      );
    });
    ...
    it("should initialize provider on registration", async () => {
      ...
      // Provider should be initialized (mock provider doesn't throw)
    ...
    it("should emit provider-registered event", async () => {
      ...
      registry.on("provider-registered", (event: ProviderEvent) => {
        eventEmitted = true;
    ...
    it("should emit provider-unregistered event", async () => {
      ...
      registry.on("provider-unregistered", (event: ProviderEvent) => {
        eventEmitted = true;
  ...
  describe("Provider Management", () => {
    ...
    it("should get only enabled providers", async () => {
      ...
      assert.strictEqual(
        ...
        1,
        "Should have 1 enabled provider",
      );
    ...
    it("should emit events when enabling/disabling", async () => {
      ...
      registry.on("provider-enabled", (event: ProviderEvent) => {
        enableEvent = true;
      });

      registry.on("provider-disabled", (event: ProviderEvent) => {
        disableEvent = true;
      });
  ...
  describe("Single Provider Operations", () => {
    ...
    it("should throw error for disabled provider", async () => {
      ...
      await assert.rejects(
        ...
        ProviderUnavailableError,
        "Should throw for disabled provider",
      );
    });
  ...
  describe("Multi-Provider Operations", () => {
    it("should fetch indexes from all providers", async () => {
      ...
      assert.ok(
        indexes.some((idx) => idx.provider === "multi-1"),
        "Should include first provider",
      );
      assert.ok(
        indexes.some((idx) => idx.provider === "multi-2"),
        "Should include second provider",
      );
    });
    ...
    it("should handle partial failures in fetchAllIndexes", async () => {
      ...
      assert.ok(
        indexes.some((idx) => idx.provider === "good-provider"),
    ...
    it("should try providers in priority order for fetchResourceAny", async () => {
      ...
      // High priority fails, should fall back to low priority
  ...
  describe("Health Monitoring", () => {
    ...
    it("should auto-disable unhealthy providers after consecutive failures", async () => {
      ...
      // Provider should be auto-disabled
      assert.strictEqual(
        provider.enabled,
        false,
        "Provider should be auto-disabled after 3 failures",
      );
    });
    ...
    it("should reset failure counter on success", async () => {
      ...
      // Fail again - should not auto-disable yet
      provider.setFailNextOperation(true);
      try {
        await registry.fetchIndex("reset-test");
      } catch (e) {}
      ...
      assert.strictEqual(
        provider.enabled,
        true,
        "Provider should still be enabled",
      );
    });
    ...
    it("should emit provider-error event on failures", async () => {
      ...
      registry.on("provider-error", (event: ProviderEvent) => {
  ...
  describe("Statistics", () => {
    ...
    it("should get aggregate statistics", async () => {
      ...
      assert.strictEqual(
        aggStats.totalProviders,
        2,
        "Should count total providers",
      );
      assert.strictEqual(
        aggStats.enabledProviders,
        2,
        "Should count enabled providers",
      );
      assert.ok(
    ...
    it("should aggregate statistics across all providers", async () => {
      ...
      assert.ok(
        aggStats.aggregate.totalRequests >= 3,
        "Should aggregate all requests",
      );
      assert.ok(
        aggStats.aggregate.avgResponseTime > 0,
    ...
    it("should count healthy providers correctly", async () => {
      ...
      assert.ok(
        aggStats.healthyProviders >= 1,
        "Should count healthy providers",
      );
    });
  });
  ...
  describe("Integration with LocalProvider", () => {
    ...
    it("should handle mixed mock and real providers", async () => {
      ...
      // Local should come first (priority 0)
      assert.strictEqual(providers[0].name, "local");
