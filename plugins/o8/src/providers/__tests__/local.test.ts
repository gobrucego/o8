 ...
 *
 * Tests the LocalProvider implementation including:
...
describe("LocalProvider", () => {
  ...
  describe("Lifecycle", () => {
    ...
    it("should fail initialization if resources path doesn't exist", async () => {
      const badProvider = new LocalProvider(
  ...
  describe("fetchIndex()", () => {
    ...
    it("should include valid resource metadata", async () => {
      ...
      assert.ok(
        typeof resource.estimatedTokens === "number",
        "EstimatedTokens should be a number",
      );
    ...
    it("should cache index results", async () => {
      ...
      assert.strictEqual(
        index1.version,
        index2.version,
        "Cached index should be identical",
      );
      assert.ok(
    ...
    it("should include all expected categories", async () => {
      ...
      for (const category of expectedCategories) {
        assert.ok(
          index.categories.includes(category),
  ...
  describe("fetchResource()", () => {
    ...
    it("should cache fetched resources", async () => {
      ...
      // First fetch
      const resource1 = await provider.fetchResource(
        firstResource.id,
        firstResource.category,
      );
      const stats1 = provider.getStats();
      ...
      const resource2 = await provider.fetchResource(
        firstResource.id,
        firstResource.category,
      );
      const stats2 = provider.getStats();
    ...
    it("should throw ResourceNotFoundError for invalid category", async () => {
      ...
      await assert.rejects(
        ...
        ResourceNotFoundError,
        "Should throw ResourceNotFoundError for invalid category",
      );
    });
  });
    ...
    it("should filter by required tags", async () => {
      ...
      for (const result of response.results) {
        const hasTags = requiredTags.every((tag) =>
          result.resource.tags.includes(tag),
        );
  ...
  describe("healthCheck()", () => {
    ...
    it("should include metrics", async () => {
      ...
      assert.ok(
        health.metrics.successRate >= 0 && health.metrics.successRate <= 1,
        "Success rate should be between 0 and 1",
      );
      assert.ok(
        typeof health.metrics.avgResponseTime === "number",
  ...
  describe("getStats()", () => {
    ...
    it("should return statistics", () => {
      ...
      assert.ok(
        typeof stats.cachedRequests === "number",
        "Should have cached requests",
      );
    ...
    it("should track requests accurately", async () => {
      ...
      assert.ok(
        statsAfter.successfulRequests > statsBefore.successfulRequests,
    ...
    it("should track resources fetched", async () => {
      ...
      assert.ok(
        statsAfter.resourcesFetched > statsBefore.resourcesFetched,
        "Resources fetched should increase",
      );
      assert.ok(
        statsAfter.tokensFetched > statsBefore.tokensFetched,
        "Tokens fetched should increase",
      );
    });
    ...
    it("should calculate average response time", async () => {
      ...
      assert.ok(
        ...
        "Average response time should be > 0",
      );
      assert.ok(
  ...
  describe("resetStats()", () => {
    ...
    it("should reset statistics", async () => {
      ...
      const statsBefore = provider.getStats();
      assert.ok(
        statsBefore.totalRequests > 0,
        "Should have requests before reset",
      );
      ...
      assert.strictEqual(
        statsAfter.successfulRequests,
        0,
        "Successful requests should be 0",
      );
      assert.strictEqual(
        statsAfter.failedRequests,
        0,
        "Failed requests should be 0",
      );
      assert.strictEqual(
        statsAfter.cachedRequests,
        0,
        "Cached requests should be 0",
      );
      assert.strictEqual(
        statsAfter.resourcesFetched,
        0,
        "Resources fetched should be 0",
      );
      assert.ok(
        statsAfter.statsResetAt > statsBefore.statsResetAt,
        "Reset timestamp should be updated",
      );
    });
  });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle errors gracefully", async () => {
      ...
      try {
        ...
        assert.ok(error instanceof ResourceNotFoundError);
        const stats = provider.getStats();
        // ResourceNotFoundError doesn't count as failed request
    ...
    it("should track failed requests in stats", async () => {
      ...
      assert.ok(
        statsAfter.failedRequests >= statsBefore.failedRequests,
  ...
  describe("Caching", () => {
    ...
    it("should allow disabling cache", async () => {
      ...
      assert.strictEqual(
        stats2.cachedRequests,
        stats1.cachedRequests,
  ...
  describe("Performance", () => {
    ...
    it("should load index in reasonable time", async () => {
      ...
      assert.ok(
        duration < 5000,
        `Index load should be fast (<5s), took ${duration}ms`,
      );
    });
    ...
    it("should perform search quickly", async () => {
      ...
      assert.ok(
        duration < 1000,
        `Search should be fast (<1s), took ${duration}ms`,
      );
    });
