...
import {
  ...
  RepositoryStructure,
  RepositoryIndex,
  GitHubResourceMetadata,
  CachedResource,
  CacheStats,
  GitHubErrorResponse,
  RepositoryScanResult,
  KNOWN_DIRECTORY_STRUCTURES,
  ResourceCategory,
  GitHubFetchOptions,
...
export class GitHubProvider implements ResourceProvider {
  ...
  // Caching
  private indexCache: LRUCache<string, RepositoryIndex>;
  private resourceCache: LRUCache<string, CachedResource>;
  private treeCache: LRUCache<string, GitHubTreeResponse>;

  // Rate limiting
  private rateLimit: GitHubRateLimit | null = null;
  private rateLimitCheckedAt: Date | null = null;
  ...
  // Repository indexes
  private repositories: Map<string, RepositoryIndex> = new Map();
  ...
  constructor(config: GithubProviderConfig, logger: Logger) {
    ...
    this.indexCache = new LRUCache<string, RepositoryIndex>({
      max: 100, // Max 100 repository indexes
      ttl: cacheTTL,
    });

    this.resourceCache = new LRUCache<string, CachedResource>({
      max: 1000, // Max 1000 resources
      ttl: config.cacheTTL || DEFAULT_CACHE_TTL.resource,
    });

    this.treeCache = new LRUCache<string, GitHubTreeResponse>({
  ...
  async initialize(): Promise<void> {
    ...
    this.logger.info(
      `GitHubProvider initialized with ${this.config.repos.length} repositories`,
    );
  }
  ...
  async fetchIndex(): Promise<RemoteResourceIndex> {
    ...
    try {
      ...
      const scanPromises = this.config.repos.map((repo) =>
      ...
      const successfulScans: RepositoryScanResult[] = [];
      const failedScans: { repo: string; error: string }[] = [];
      ...
      const allResources: RemoteResourceMetadata[] = [];
      const categories = new Set<RemoteResourceMetadata["category"]>();
      ...
      for (const [repoKey, repoIndex] of this.repositories.entries()) {
        for (const resource of repoIndex.resources) {
          ...
          const metadata: RemoteResourceMetadata = {
            id: resource.id,
            ...
            estimatedTokens:
              resource.estimatedTokens || Math.ceil(resource.size / 4),
    ...
    try {
      ...
      if (parts.length < 3) {
        throw new ResourceNotFoundError(
          ...
          id,
          category,
          new Error("Invalid resource ID format"),
        );
      }
  ...
  async search(
    ...
    try {
      ...
      const allResources: GitHubResourceMetadata[] = [];
  ...
  async healthCheck(): Promise<ProviderHealth> {
    ...
    try {
      ...
      if (!this.initialized) {
        ...
        this.stats.failedRequests / Math.max(this.stats.totalRequests, 1) >
        0.5
      ) {
        status = "degraded";
      } else {
  ...
  private async scanRepository(
    ...
    try {
      ...
      const resources: GitHubResourceMetadata[] = mdFiles.map((file) => {
  ...
  private detectStructure(
    ...
    const orchestr8Dirs = new Set(
      Object.keys(KNOWN_DIRECTORY_STRUCTURES.orchestr8),
    );
    const templateDirs = new Set(
      Object.keys(KNOWN_DIRECTORY_STRUCTURES["claude-code-templates"]),
    );

    const hasOrchstr8 = Array.from(orchestr8Dirs).some((dir) =>
      directories.has(dir),
    );
    const hasTemplate = Array.from(templateDirs).some((dir) =>
      directories.has(dir),
    );

    let structureType: RepositoryStructure["structureType"];
    let categoryMap: Record<string, ResourceCategory>;

    if (hasOrchstr8) {
      structureType = "orchestr8";
      categoryMap = KNOWN_DIRECTORY_STRUCTURES.orchestr8 as any;
    } else if (hasTemplate) {
      structureType = "claude-code-templates";
      categoryMap = KNOWN_DIRECTORY_STRUCTURES["claude-code-templates"] as any;
    } else {
      structureType = "flat";
      categoryMap = {};
    }
    ...
    const resourceDirs: RepositoryStructure["directories"] = [];
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (error instanceof Error && error.name === "AbortError") {
          this.logger.warn(
            `Request timeout (attempt ${attempt + 1}/${maxRetries + 1})`,
            { url },
          );
        } else {
          this.logger.warn(
  ...
  private calculateScore(
    resource: GitHubResourceMetadata,
    keywords: string[],
    options?: SearchOptions,
  ): number {
    let score = 0;
    ...
    const capabilities = (resource.capabilities || []).map((c) =>
      c.toLowerCase(),
    );
   ...
   */
  private parseRepository(repoString: string): RepositoryIdentifier {
  ...
  private buildRemoteResource(
    ...
    return {
      ...
      estimatedTokens:
        cached.frontmatter.estimatedTokens ||
