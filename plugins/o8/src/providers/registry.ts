...
export class ProviderRegistry extends EventEmitter {
   ...
   * ```
   */
  async register(provider: ResourceProvider): Promise<void> {
   ...
   */
  getProviders(enabledOnly = false): ResourceProvider[] {
   ...
   */
  async fetchIndex(providerName: string): Promise<RemoteResourceIndex> {
   ...
   */
  async fetchAllIndexes(): Promise<RemoteResourceIndex[]> {
    const providers = this.getProviders(true);
    const results = await Promise.allSettled(
      providers.map((p) =>
        this.executeWithErrorHandling(p, () => p.fetchIndex(), "fetchIndex"),
      ),
    );

    return results
      .filter((r): r is PromiseFulfilledResult<RemoteResourceIndex> => r.status === "fulfilled")
      .map((r) => r.value);
  }
  ...
  async checkHealth(name: string): Promise<ProviderHealth> {
    ...
    try {
      ...
      if (health.status === "healthy") {
        ...
        if (
          this.config.autoDisableUnhealthy &&
  ...
  async checkAllHealth(): Promise<Map<string, ProviderHealth>> {
    const providers = Array.from(this.providers.values());
    const results = await Promise.allSettled(
      providers.map((p) => this.checkHealth(p.name)),
    );
   ...
   * @private
   */
  private getEnabledProvider(name: string): ResourceProvider {
    const provider = this.providers.get(name);
    if (!provider) {
      throw new ProviderUnavailableError("registry", `Provider ${name} not found`);
    }
  ...
  private async executeWithErrorHandling<T>(
    ...
    try {
      ...
      this.emitEvent({
        ...
        provider: provider.name,
        timestamp: new Date(),
        data: { operation: operationName, error: (error as Error).message },
      });
   ...
   * @private
   */
  private calculateFacets(results: Array<{ resource: ResourceFragment }>) {
