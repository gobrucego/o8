...
- **Dynamic loading:** Loads expertise per phase using `o8://` URIs
...
o8://agents/typescript-api-development
o8://agents/typescript-testing
# Loads ~2000 tokens immediately

✅ Good: Load per phase
Phase 1: o8://match?query=requirements+analysis&maxTokens=800
Phase 2: o8://agents/typescript-core
Phase 3: o8://agents/typescript-api-development
Phase 4: o8://skills/testing-integration-patterns
...
└─ Example: o8://skills/match?query=requirement+analysis&maxTokens=1000
...
└─ Example: o8://match?query=${tech}+architecture&categories=agent,pattern&maxTokens=1500
...
└─ Example:
    o8://agents/${tech}-core (prerequisite)
    o8://agents/${tech}-${specialization}
    o8://examples/${specific-pattern}

Phase 4 (Testing): 800-1200 tokens
├─ Load: Testing skills + validation patterns
├─ Strategy: Only testing-specific knowledge
└─ Example: o8://skills/match?query=testing+${tech}&maxTokens=1000
...
o8://match?query=python+api&maxTokens=1500

# Instead of unlimited loading that might fetch 3000+ tokens
o8://match?query=python+api
```

2. **Category filtering:**
```markdown
# Phase 1: Only load patterns and skills (no examples yet)
o8://match?query=api+design&categories=pattern,skill&maxTokens=1000

# Phase 3: Now load examples too
o8://match?query=api+implementation&categories=agent,example&maxTokens=2000
```
...
o8://agents/match?query=${technology}+${framework}

# Phase 3: Very specific (loads core + multiple specializations + examples)
o8://match?query=${technology}+${framework}+${feature}&categories=agent,skill,example
```
...
**→ Load:** o8://skills/match?query=feature+planning+requirements&maxTokens=1000
...
**→ Load:**
- o8://agents/${tech}-core (prerequisite)
- o8://patterns/match?query=${architecture}&maxTokens=800

Load core language agent + architectural patterns.

## Phase 3: Implementation (50-80%) - Budget: 2500 tokens
**→ Load:**
- o8://agents/${tech}-core (prerequisite, may already be cached)
- o8://agents/${tech}-${specialization}
- o8://examples/match?query=${tech}+${pattern}&maxTokens=1000

Now load specialized implementation knowledge and examples.

## Phase 4: Testing (80-100%) - Budget: 1000 tokens
**→ Load:** o8://skills/match?query=testing+${tech}&maxTokens=1000
...
**→ JIT Load:** o8://match?query=...&maxTokens=...
...
**→ Load:** o8://agents/typescript-core
**→ Load:** o8://patterns/api-design-principles
```
...
**→ Load:** o8://match?query=${technology}+${domain}&categories=agent,skill&maxTokens=1200

## Phase 3: Implementation (50-80%)
**→ Load:** o8://agents/match?query=${language}+${framework}&maxTokens=2000
```
...
**→ Load:** o8://skills/match?query=requirement+analysis&maxTokens=800
# Limits to ~800 tokens

## Phase 3: Implementation (40-80%)
**→ Load:** o8://match?query=${tech}+development&categories=agent,skill,example&maxTokens=2500
...
o8://match?query=api+development&categories=agent,skill

# Load all relevant content
o8://match?query=microservices&categories=agent,skill,pattern,example
```
...
**→ Load:** o8://match?query=${technology}+${domain}&maxTokens=1000
```
...
**→ Load:** o8://skills/match?query=feature+planning&maxTokens=800
...
**→ Load:** o8://agents/match?query=${technology}+development&maxTokens=2000
...
**→ Load:** o8://skills/match?query=testing+${technology}&maxTokens=800
...
Expected in Phase 2:
o8://agents/match?query=typescript+development
...
**→ Load:** o8://skills/match?query=debugging+root+cause&maxTokens=800
...
**→ Load:** o8://match?query=${bug-description}+fix&categories=agent,skill,example&maxTokens=1500
...
**→ Load:** o8://skills/match?query=testing+integration&maxTokens=600
...
**→ Load:** o8://skills/match?query=technology+evaluation+research&maxTokens=1000
...
**→ Load:** o8://match?query=${technology}+${use-case}&categories=agent,pattern,example&maxTokens=2000
...
**→ Load:** o8://patterns/match?query=technology+selection&maxTokens=800
...
**→ Load:** o8://agents/match?query=agent+designer&maxTokens=800
...
**→ Load:** o8://skills/match?query=fragment+metadata&maxTokens=600
...
**→ Load:** o8://match?query=${domain}&categories=agent,skill,example&maxTokens=2000
...
**→ Load:** o8://skills/match?query=fragment+discovery+testing&maxTokens=600
...
Discovery: o8://workflows/match?query=...
```
