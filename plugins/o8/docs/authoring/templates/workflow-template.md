...
**→ JIT Load:** o8://match?query=${relevant-query}&categories=${relevant-categories}&maxTokens=${token-budget}
...
**→ JIT Load:** o8://agents/match?query=${technology}+${domain}&maxTokens=${token-budget}
...
**→ JIT Load:** o8://match?query=${specific-need}&categories=agent,skill,example&maxTokens=${token-budget}
...
└─ o8://match?query=research+analysis&maxTokens=1000

Phase 2 (Design): 1200-1800 tokens
└─ o8://match?query=${domain}+architecture&maxTokens=1500

Phase 3 (Implementation): 2000-2500 tokens
└─ o8://match?query=${tech}+${feature}&categories=agent,skill,example&maxTokens=2500

Phase 4 (Testing): 800-1200 tokens
└─ o8://skills/match?query=testing+${tech}&maxTokens=1000
```
...
```markdown
o8://match?query=${technology}+${domain}&categories=agent,skill
o8://agents/match?query=${language}+${framework}
```

**With constraints:**
```markdown
o8://match?query=${need}&categories=agent,skill&maxTokens=1500&minScore=20
```
...
**→ Load:** o8://match?query=${technology}+${feature-description}
```
...
- **Track A - Backend:** API implementation
  └─ Load: o8://agents/backend-${technology}
- **Track B - Frontend:** UI implementation
  └─ Load: o8://agents/frontend-${framework}
- **Track C - Database:** Schema design
  └─ Load: o8://patterns/database-design
