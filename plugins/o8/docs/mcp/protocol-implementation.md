...
- Enables URIs like `o8://agents/match?query=api&maxTokens=2000`
...
o8://match?query={{task}}&maxTokens=3000
```
...
server.registerResource(
  ...
  resource.uri,            // "o8://agents/typescript-core"
  {
...
Matches:  o8://agents/match?query=typescript+api&maxTokens=2000
```
...
```
1. Client requests URI (e.g., o8://agents/match?query=api)
...
- `o8://agents/typescript-core` → Cached
- `o8://agents/match?query=api&maxTokens=2000` → Cached
- `o8://agents/match?query=database&maxTokens=2000` → Separate cache entry
    ...
    "resources": [
      {
        ...
        "description": "Resource: o8://agents/typescript-core",
        "mimeType": "text/markdown"
      },
      {
        "uri": "o8://agents/match{+rest}",
        "name": "agents-dynamic",
...
```json
{
  ...
  "error": {
    "code": -32602,
    "message": "Resource not found: o8://agents/non-existent"
  }
}
```
