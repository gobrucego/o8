...
o8://match?query=testing+unit+integration&mode=index&maxResults=5
...
# → o8://examples/infrastructure/sre-slo-configuration
# → o8://examples/infrastructure/sre-incident-management

# Load examples only when needed:
o8://examples/infrastructure/sre-slo-configuration  # 800 tokens
...
o8://match?query=circuit+breaker+resilience&mode=index&maxResults=5
...
o8://match?query=typescript+api+testing&mode=catalog

# Review (~100 tokens), then load selectively
o8://agents/typescript-core       # 650 tokens
o8://skills/api-design-rest       # 720 tokens
o8://examples/express-minimal-api # 500 tokens
...
o8://match?query=authentication+jwt+security&categories=skills,patterns,examples&mode=catalog&maxResults=15
...
o8://match?query=retry+backoff&mode=index&maxResults=5

# Full mode (complete content)
o8://match?query=python+async&mode=full&maxTokens=2500
```
...
o8://match?query=circuit+breaker+timeout&mode=index
```
...
o8://match?query=testing+patterns&mode=catalog&maxResults=10
```
