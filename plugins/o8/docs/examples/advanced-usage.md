...
**Query:**
```
o8://match?query=typescript+react+hooks+api+postgresql+authentication&mode=catalog&maxResults=20
```
...
o8://agents/react-hooks-patterns

# Load specific skills
o8://skills/api-design-rest
o8://skills/database-postgresql

# Load examples
o8://examples/express-jwt-auth
o8://examples/react-auth-flow
```
...
**Step 1 - Broad query:**
```
o8://match?query=testing&mode=catalog&maxResults=20
```
...
```
o8://match?query=typescript+testing&mode=catalog&maxResults=15
```
...
```
o8://match?query=typescript+testing+unit+jest+mocking&mode=catalog&maxResults=10
```
...
o8://match?query=express+middleware+jwt+validation+error+handling
...
o8://match?query=integration+testing+api+mocking+database
```
...
o8://match?query=kubernetes+deployment+production&mode=catalog&maxResults=15
...
**Approach:**
```
o8://match?query=retry+exponential+backoff+circuit+breaker&mode=index&maxResults=5
```
...
```
o8://skills/api-design-rest  # 720 tokens
o8://skills/error-handling-resilience  # 650 tokens
```

**Phase 3 - Load examples only when needed (on-demand):**
```
# Agent with progressive loading
o8://agents/mlops-specialist  # 220 tokens core

# Returns core + on-demand example references:
# → o8://examples/ml/mlops-pipeline-kubeflow
# → o8://examples/ml/mlops-model-versioning
...
**Query:**
```
o8://match?query=authentication+jwt+security&categories=agents,skills,patterns,examples&mode=catalog&maxResults=20
```
...
o8://examples/match?query=authentication+jwt&mode=catalog&maxResults=10

# If insufficient, expand to patterns
o8://match?query=authentication+jwt&categories=examples,patterns&mode=catalog
```
...
o8://skills/match?query=typescript+testing+error+handling&mode=catalog

# Level 3: Specific examples
o8://examples/match?query=typescript+jest+mocking&mode=catalog
```
...
**Implementation:**
```
o8://match?query=circuit+breaker+timeout+retry&mode=index&maxResults=5
```
...
```
o8://match?query=retry+exponential+backoff+circuit+breaker+timeout&mode=index
```
...
o8://match?query=kubernetes+helm+deployment&mode=index&maxResults=5

# If insufficient results, try fuzzy catalog
o8://match?query=kubernetes+helm+deployment+production&mode=catalog&maxResults=10
```
...
o8://match?query=typescript+testing&mode=catalog
...
Promise.all([
  loadResource('o8://agents/typescript-core'),
  loadResource('o8://skills/api-design-rest'),
  loadResource('o8://examples/express-minimal-api')
])
```
...
o8://match?query=deployment+production+rollback&mode=index

# Cache-friendly queries (exact same URI)
o8://workflows/workflow-deploy
```
...
try {
  // Try specific query
  result = query('o8://match?query=specific+keywords&mode=index')
} catch (error) {
  // Fallback to broader fuzzy query
  result = query('o8://match?query=broader+terms&mode=catalog')
}

// Ultimate fallback: Static resource
if (!result) {
  result = query('o8://agents/default-agent')
}
```
...
async function generateFeature(description, techStack) {
  ...
  const workflow = await loadResource('o8://workflows/workflow-add-feature')
...
async function executeWorkflowWithJIT(workflow, phases, description) {
  for (const [phaseName, config] of Object.entries(phases)) {
    ...
    const expertise = await query(
      `o8://match?query=${config.query}&maxTokens=${config.maxTokens}&mode=full`
    )
...
2. Load relevant patterns
   o8://patterns/match?query=...
   ↓
3. Load code examples
   o8://examples/match?query=...
   ↓
...
const agents = await query('o8://agents/match?query=&mode=catalog&maxResults=100')
...
describe('API Tests', () => {
  beforeAll(async () => {
    ...
    testPatterns = await loadResource('o8://skills/testing-integration')
  })
