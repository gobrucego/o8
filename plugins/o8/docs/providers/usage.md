...
**Local filesystem**:
```markdown
**Agent:** `o8://agents/typescript-developer`
**Skill:** `o8://skills/error-handling`
**Pattern:** `o8://patterns/security/authentication`
```
...
**Basic search**:
```
o8://match?query=typescript+rest+api
```

**With filters**:
```
o8://match?query=python+async&categories=agent,skill&maxTokens=2500
```

**Required tags**:
```
o8://match?query=authentication&tags=security,jwt&maxResults=5
```
...
o8://match?query=typescript+rest+api+authentication&maxTokens=3000&categories=agent,skill
...
o8://match?query=jwt+authentication+security&tags=security&maxTokens=2000
...
o8://match?query=python+async+error+handling

<!-- Less flexible: Hard-coded reference -->
o8://agents/python-developer
```
...
o8://match?query=typescript+express+validation+zod&categories=example

// Vague query (worse)
o8://match?query=typescript
```
