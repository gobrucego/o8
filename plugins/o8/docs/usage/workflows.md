...
Analyzes requirements: "TypeScript", "REST API"
   ↓
Dynamic matching: o8://match?query=typescript+rest+api&maxTokens=2500
   ↓
Assembles expertise:
  - typescript-core
...
```
→ Load: o8://match?query=requirements+analysis+architecture+design&maxTokens=1200
...
```
→ Load: o8://match?query=typescript+project+initialization+tooling&maxTokens=1000
...
```
→ Load: o8://match?query=typescript+rest+api+implementation&maxTokens=3500
...
```
→ Load: o8://match?query=typescript+testing+quality+security&maxTokens=2000
...
```
→ Load: o8://match?query=${deployment-platform}+cicd+infrastructure&maxTokens=1800
...
4. Dynamic resource loading
   o8://match?query=typescript+rest+api&maxTokens=2500
...
o8://match?query=relevant+keywords&maxTokens=2500
