...
│   └── Examples via o8:// URIs ← Load on-demand
│       Example: sre-specialist
│       - Core: 180 tokens
│       - Examples: o8://examples/infrastructure/sre-slo-configuration
...
│   └── Working code referenced by o8:// URIs
│
├── guides/           # Setup guides
...
- **On-demand examples**: Load via o8:// URIs when needed
- **Token efficiency**: 52-82% savings vs loading all content
- **Real example**: rust-expert-core (700 tokens) + advanced patterns (o8://agents/rust-expert-advanced) loaded when needed

### Loading Modes

| Mode | Use Case | Syntax | Tokens | Speed |
|------|----------|--------|--------|-------|
| **Static** | Known resource | `o8://category/id` | Exact | <5ms |
| **Catalog** | Discovery | `o8://match?query=...&mode=catalog` | 50-120 | ~15ms |
| **Index** | Fast lookup | `o8://match?query=...&mode=index` | 50-120 | ~5ms |
| **Full** | Ready-to-use | `o8://match?query=...&mode=full` | 800-3000 | ~15ms |
...
```
o8://match?query=retry+exponential+backoff&mode=index&maxResults=5
→ Returns top 5 in 5-10ms with keyword matching
```

**Catalog discovery:**
```
o8://match?query=typescript+api+authentication&mode=catalog&maxResults=15
→ Returns 15 ranked results in 15-20ms with semantic scoring
```

**Full content assembly:**
```
o8://match?query=python+async+patterns&mode=full&maxTokens=2500
...
o8://agents/python-async-fundamentals
o8://agents/go-concurrency-patterns
```

**Dynamic loading:**
```
o8://agents/match?query=typescript+generics+api
```
...
**Example URIs:**
```
o8://skills/error-handling-resilience
o8://skills/api-design-rest
o8://skills/database-optimization
```

**Dynamic loading:**
```
o8://skills/match?query=retry+exponential+backoff
```
...
**Example URIs:**
```
o8://patterns/microservices-architecture
o8://patterns/security-auth-jwt
o8://patterns/event-sourcing
```

**Dynamic loading:**
```
o8://patterns/match?query=microservices+api+gateway
```
...
o8://examples/fastapi-async-crud
o8://examples/docker-multistage-nodejs
```

**Dynamic loading:**
```
o8://examples/match?query=express+middleware+auth
```
...
**Example URIs:**
```
o8://guides/aws-eks-setup
o8://guides/docker-compose-development
o8://guides/github-actions-cicd
```

**Dynamic loading:**
```
o8://guides/match?query=kubernetes+aws+setup
```
...
**Example URIs:**
```
o8://best-practices/code-review-checklist
o8://best-practices/security-owasp-top10
o8://best-practices/api-versioning
```

**Dynamic loading:**
```
o8://best-practices/match?query=api+design+standards
```
...
o8://workflows/workflow-add-feature
o8://workflows/workflow-code-review
```

**Dynamic loading:**
```
o8://workflows/match?query=deployment+production+rollback
```
...
**URI Format:**
```
o8://[category]/match?query=<search>&[options]
```
...
```
o8://match?query=typescript+testing&mode=catalog&maxResults=10
```
...
2. Integration Testing Strategies (Score: 72)
   URI: o8://skills/testing-integration
   Tokens: 680
...
```
o8://match?query=typescript+developer&mode=full&maxTokens=2000&categories=agents
```
...
**Basic query:**
```
o8://match?query=error+handling
```

**With mode:**
```
o8://match?query=error+handling&mode=index
```

**With token budget:**
```
o8://match?query=typescript+api&mode=full&maxTokens=2500
```

**With category filter:**
```
o8://match?query=testing&categories=skills,patterns
```

**With result limit:**
```
o8://match?query=kubernetes&mode=catalog&maxResults=5
```

**With score threshold:**
```
o8://match?query=security&minScore=20
```

**With required tags:**
```
o8://match?query=api&tags=typescript,rest
```

**Category-specific:**
```
o8://agents/match?query=build+graphql+api
```
...
Better than:
```
o8://match?query=typescript&mode=full
```

**2. Use catalog mode for discovery:**
```
# First: Discover
o8://match?query=testing+patterns&mode=catalog

# Then: Load specific
o8://skills/testing-integration
```

**3. Use index mode for speed:**
```
o8://match?query=retry+backoff&mode=index
```
...
```
o8://match?query=typescript+api+testing&mode=full&maxTokens=2500
```

**Deep expertise** (3000-5000 tokens):
```
o8://match?query=microservices+kubernetes+deployment&mode=full&maxTokens=5000
```
...
o8://match?query=kubernetes+deployment&mode=catalog&maxResults=10
...
o8://guides/kubernetes-deployment
o8://patterns/helm-charts
```
...
```
o8://match?query=circuit+breaker+timeout&mode=index&maxResults=5
```
...
o8://examples/nodejs-microservice
```
...
```
o8://match?query=authentication+jwt&categories=skills,patterns,examples&mode=catalog
```
...
```
# Too specific
o8://match?query=typescript+generic+type+constraints+inference+advanced&minScore=50

# Better
o8://match?query=typescript+generics&minScore=10
```
...
# Better
o8://match?query=rest+api+authentication+jwt&categories=skills,patterns
```
