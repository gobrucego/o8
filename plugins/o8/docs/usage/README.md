...
# Additional examples loaded on-demand via o8:// URIs:
→ o8://examples/typescript-generics-example
→ o8://examples/typescript-async-patterns
→ o8://skills/typescript-testing-jest
...
```
o8://match?query=typescript+api+auth&maxTokens=2500&mode=catalog
```
...
1. **Analysis & Design** (0-20%)
   ...
   **→ Load (JIT):**
   ```
   o8://match?query=codebase+analysis+feature+design+${tech-stack}&maxTokens=1000
   ```
...
2. **Implementation** (20-70%)
   ...
   **→ Load (JIT):**
   ```
   o8://match?query=${tech-stack}+authentication+jwt+implementation&maxTokens=2500
   ```
...
3. **Quality** (70-90%)
   ...
   **→ Load (JIT):**
   ```
   o8://match?query=${tech-stack}+testing+security+quality&maxTokens=1500
   ```
...
4. **Integration & Deploy** (90-100%)
   ...
   **→ Load (JIT - CONDITIONAL):**
   ...
   o8://match?query=${platform}+deployment+feature+flags&maxTokens=1000
   ```
...
|----------|---------|-------------|
| **agents** | AI agent definitions | `o8://agents/typescript-core` |
| **skills** | Reusable techniques | `o8://skills/error-handling-resilience` |
| **patterns** | Design patterns | `o8://patterns/microservices-architecture` |
| **examples** | Code examples | `o8://examples/express-jwt-auth` |
| **guides** | Setup guides | `o8://guides/aws-eks-setup` |
| **workflows** | Process templates | `o8://workflows/workflow-new-project` |
...
o8://examples/express-minimal-api
```
...
```
o8://match?query=typescript+api&mode=full&maxTokens=2500&maxResults=3
```
...
```
o8://match?query=typescript+rest+api+authentication&mode=catalog
```
...
```
o8://match?query=python+fastapi+async&mode=full&maxTokens=2500
```

**Category-specific search:**
```
o8://agents/match?query=build+graphql+api&mode=catalog
```

**Multi-category search:**
```
o8://match?query=testing+strategies&categories=skills,patterns&mode=catalog
```
...
```
o8://skills/match?query=error+handling&mode=catalog
```
