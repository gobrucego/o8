...
9. Register dynamic resource templates
   ├─ Category-specific: o8://{category}/match{+rest}
   └─ Global: o8://match{+rest}
...
**Dynamic Templates:**
```typescript
const templateUri = `o8://${category}/match{+rest}`;
...
**Dynamic URI:**
```
o8://category/match?query=typescript+api&maxTokens=2000
         ↑        ↑     ↑              ↑
...
**Global Dynamic URI:**
```
o8://match?query=python+async&categories=skill,example
         ↑     ↑              ↑
...
parse(uri: string): ParsedURI {
  // Validate protocol
  if (!uri.startsWith("o8://")) {
    throw new Error("Invalid URI protocol");
  }

  // Remove protocol
  const withoutProtocol = uri.substring("o8://".length);
...
```
Input:
  "o8://agents/match?query=typescript+api&maxTokens=2500&tags=async&mode=catalog"
...
```json
{
  ...
  "error": {
    ...
    "data": {
      "details": "Resource not found: o8://agents/nonexistent"
    }
  }
}
```
...
5. Cache with key `o8://agents/typescript-developer`
6. Return content
...
**Request:**
```
URI: o8://match?query=typescript+async+error&maxTokens=2500&mode=catalog
```
...
**Request:**
```
URI: o8://match?query=typescript+async+error&mode=index
```
...
```json
{
  ...
  "index": {
    "scenario-a1b2c3d4e5f6": {
      ...
      "uri": "o8://agents/typescript-async-patterns",
      "category": "agent",
      "estimatedTokens": 800,
      "relevance": 100
    },
    "scenario-f6e5d4c3b2a1": {
      ...
      "uri": "o8://skills/error-handling-async",
      "category": "skill",
      "estimatedTokens": 650,
      "relevance": 100
    }
...
```
Key: Full URI
Example: "o8://agents/typescript-developer"
```

**Dynamic Resources:**
```
Key: Full query URI (including all parameters)
Example: "o8://match?query=typescript+async&maxTokens=2500&mode=catalog"
```
