...
Incorrect format:
```
o8://agents/typescript-core.md  ❌ (no extension)
o8://typescript-core            ❌ (missing category)
o8://agents/typescript_core     ❌ (underscore instead of hyphen)
```
...
loader.getResource('o8://agents/typescript-core').then(console.log);
"
```
...
o8://match?query=typescript+express+jwt+authentication+redis

# Better (broader)
o8://match?query=typescript+authentication
```
...
o8://match?query=typescript+api&maxTokens=5000
```
...
```bash
# Too restrictive
o8://match?query=python&categories=example

# Try without filter
o8://match?query=python
```
...
o8://match?query=api&tags=graphql,typescript

# Try without required tags
o8://match?query=api+graphql+typescript
```
...
# Specific
o8://match?query=rest+api+authentication+typescript
```
...
o8://match?query=testing&categories=example,skill
```
...
o8://match?query=async+patterns&tags=typescript
```
...
o8://match?query=typescript&maxTokens=1500
```
...
```
[debug] Loading resource: o8://agents/typescript-core
[debug] Cache hit: o8://agents/typescript-core
...
```
[error] Failed to load resource: o8://agents/nonexistent
