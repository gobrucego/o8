...
Both systems use the same `o8://` URI protocol with query parameters to specify search criteria.
...
**Example:**
```
o8://match?query=typescript+api+patterns
...
**Example:**
```
o8://match?query=typescript+api&mode=minimal&maxResults=5
```
...
**Example:**
```
o8://match?query=typescript+api+patterns&mode=catalog&maxResults=10
```
...
**Example:**
```
o8://match?query=typescript+developer&mode=full&maxResults=2&maxTokens=2500
```
...
```
o8://[category]/match?query=<search-terms>&[options]
```
...
```
o8://match?query=typescript+api
```

**Category-filtered query:**
```
o8://agents/match?query=typescript+developer
o8://agents/match?query=typescript+developer&mode=index  # same (index is default)
```

**Multi-category query:**
```
o8://match?query=error+handling&categories=skills,patterns
```

**Index mode (default):**
```
o8://match?query=retry+timeout
o8://match?query=retry+timeout&mode=index&maxResults=5
```

**Minimal mode with compact JSON:**
```
o8://match?query=testing+patterns&mode=minimal&maxResults=5
```

**Catalog mode with full metadata:**
```
o8://match?query=testing+patterns&mode=catalog&maxResults=8&minScore=15
```

**Full content mode:**
```
o8://match?query=kubernetes+deployment&mode=full&maxTokens=3000
```
...
**Query:**
```
o8://match?query=typescript+api+development&mode=catalog&maxResults=10
```
...
**Query:**
```
o8://match?query=retry+timeout+exponential&mode=index&maxResults=5
```
...
**Query:**
```
o8://patterns/match?query=testing+unit+integration&mode=catalog&minScore=20
```
...
**Query:**
```
o8://match?query=error+handling+resilience&categories=skills,patterns&mode=catalog
```
