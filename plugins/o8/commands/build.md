...
```
o8://match?query=catalog&mode=index&maxResults=500
```
...
# If Python FastAPI
o8://agents/python-core
o8://agents/python-fastapi-middleware

# If React frontend
o8://agents/frontend-react-expert

# If Rust systems
o8://agents/rust-expert-core
```

**For Skills & Techniques:**
```
# If testing required
o8://skills/testing-strategies

# If API design
o8://skills/api-design-rest

# If error handling
o8://skills/error-handling-async
...
o8://patterns/phased-delivery

# Complex (50+ files, multiple domains)
o8://patterns/autonomous-organization

# Parallel independent work
o8://patterns/autonomous-parallel
```

**Dynamic Discovery:**
If you're unsure what resources exist, use fuzzy matching:
```
o8://match?query=authentication+jwt+security&categories=agent,skill&mode=index&maxResults=8&minScore=20
```
...
o8://match?query=caching+strategies+redis&mode=index&maxResults=8

# Need database optimization
o8://match?query=database+query+optimization+postgres&mode=index&maxResults=8

# Security vulnerability found
o8://match?query=security+input+validation+sql+injection&mode=index&maxResults=8
```
...
2. **Load Validation Resources if Needed**
   ...
   o8://workflows/workflow-security-audit

   # If performance critical
   o8://workflows/workflow-benchmark
   ```
...
2. Fetch: o8://agents/python-fastapi-middleware
...
o8://agents/typescript-core
o8://agents/typescript-async-patterns
...
4. Fetch: o8://skills/error-handling-async
...
```
1. Search: o8://match?query=kubernetes+deployment+scaling&categories=pattern,example
2. Review catalog results
3. Load specific fragments: o8://patterns/k8s-deployment-basic
```
...
o8://match?query=rust+async+web+server&categories=agent,example
...
```
1. Query registry: o8://registry (250 tokens)
2. Search: o8://match?query=typescript+api&mode=index (400 tokens)
...
```
1. Query: o8://match?query=typescript+api&mode=minimal (350 tokens)
...
o8://examples/typescript-rest-api-complete
```
...
o8://match?query=typescript+api&mode=index&maxResults=5
o8://match?query=keywords  # mode=index is default

# Minimal mode (ultra-compact JSON with URIs and scores)
o8://match?query=rust+async&mode=minimal&maxResults=5

# Catalog mode (full metadata, 85-92% token reduction)
o8://match?query=keywords&mode=catalog&maxResults=20&minScore=15

# Full mode (loads complete content)
o8://match?query=keywords&mode=index&maxResults=12&categories=agent,skill

# With filters
o8://match?query=rust+async&categories=agent,pattern&minScore=20

# Category-specific
o8://agents/match?query=python+fastapi&mode=index
o8://skills/match?query=error+handling+async&mode=minimal
```
